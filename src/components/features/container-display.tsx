...
import { Container } from 'lucide-react'; // Changed from MonitorPlay
    ...
    <Card className="w-full h-[calc(100vh-10rem)] shadow-2xl border-2 border-primary/30 bg-card overflow-hidden flex flex-col">
      <CardHeader className="bg-card-foreground/5 border-b border-border">
        <CardTitle className="flex items-center gap-2 text-xl text-foreground">
          <Container className="h-6 w-6 text-primary" /> {/* Changed icon */}
          Container Instance Display
        </CardTitle>
        <CardDescription className="text-muted-foreground">
          Interactive session will appear here when container is running.
        </CardDescription>
      </CardHeader>
      <CardContent className="flex-grow flex items-center justify-center p-0 relative">
        ...
        <div className="text-center p-8 z-10">
          <Container className="h-24 w-24 text-primary/40 mx-auto mb-4" /> {/* Changed icon */}
          <p className="text-2xl font-semibold text-muted-foreground">
            Container Display Area
          </p>
          <p className="text-muted-foreground mt-2">
            Start a container to begin interaction.
          </p>
        </div>
      </CardContent>
    </Card>
